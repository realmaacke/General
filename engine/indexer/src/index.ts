...
import { loadSettings, SettingsType, loadFilters, FiltersType } from "./utils.js";
...
class Main {
    ...
    constructor(settings: SettingsType | null, filter: FiltersType | null) {
    ...
    async run() {
        ...
        while (true) {
            const [stateRows]: any = await pool.execute(`SELECT dirty FROM index_state WHERE id = 1`);
    ...
    async buildIndex(pool: mysql.Pool) {
        ...
        const postingsBatch: any[] = [];
        const docLengths: any[] = [];
        const metaBatch: any[] = [];
